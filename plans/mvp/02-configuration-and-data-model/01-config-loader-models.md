...
## Goal
Add typed Pydantic models and loader functions for the versioned configuration files that drive sources, extractors, models, retrieval, evaluation, security, access, and memory policy.
...
2. Create Pydantic models with `extra="forbid"` for these versioned files: `sources.yaml`, `extractors.yaml`, `models.yaml`, `retrieval.yaml`, `evaluation.yaml`, `security.yaml`, `access.yaml`, and `memory.yaml`.
...
4. Model `config/sources.yaml` with source IDs, source type, repository or artifact URL, tracked refs, version strategy, include/exclude paths, extractor profile, source priority, visibility label, allowed groups/principals, sensitivity class, license policy, and refresh cadence.
...
10. Model `security.yaml`, `access.yaml`, and `memory.yaml` for redaction rules, prompt-injection handling, source allowlists, visibility labels, ACL policy, memory scopes, retention, and promotion rules. Memory UX is out of MVP; this step only validates configuration.
