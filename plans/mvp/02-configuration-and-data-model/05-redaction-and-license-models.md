...
4. Add `retrieval_events` as a sanitized MVP event table. Retrieval events must store query hash, sanitized query preview or token counts, trusted filters, selected IDs, ranking diagnostics JSON, redaction status, access-filter result, and nullable active index version ID, but never full unsanitized chunks or sensitive source text. Because `index_versions` is added in Step 2.6, keep this column nullable and non-blocking until that migration can add the foreign key.
