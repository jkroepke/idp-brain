...
3. In `sources.yaml`, include source ID, source type, local path or URL field, tracked refs or explicit version, version strategy, include/exclude paths, extractor profile, source priority, visibility label, allowed groups/principals, sensitivity class, license policy, and refresh cadence.
...
9. In `access.yaml`, define visibility labels, default groups/principals, source filters, sensitivity labels, license policy labels, and a default-deny fallback for unknown labels.
...
- The examples cover sources, extractors, models, retrieval, evaluation, security, access, and memory without requiring code changes for a new source.
- Active defaults use deterministic local/mock embedding and reranking.
- ACL, source allowlist, sensitivity, and license labels are present for later pre-subquery filtering.
- Apache ACE, memory UX, embedding fine-tuning, production HA, and remote model serving remain out of MVP.
