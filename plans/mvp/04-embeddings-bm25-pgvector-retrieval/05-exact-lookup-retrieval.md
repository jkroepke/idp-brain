...
- Source allowlist, license, sensitivity, redaction, and version constraints are available from configuration and request context.
...
5. Implement `ExactLookupRetriever.retrieve(query, filters, limit)` so it always applies trusted filters before lookup predicates. Until Step 4.9 centralizes filtering, build a local filtered CTE with source allowlist, corpus eligibility labels, sensitivity class, license policy status, active index version, and version scope.
...
9. Record diagnostics only after filters are applied. Do not report counts for ineligible chunks.
...
- Test that ineligible, sensitivity-disallowed, source-disallowed, and license-disallowed chunks are excluded before lookup.
