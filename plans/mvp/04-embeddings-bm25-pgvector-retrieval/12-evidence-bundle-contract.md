...
1. Define Pydantic models for `EvidenceBundle`, `EvidenceItem`, `Citation`, `AccessFilterResult`, `ConflictMarker`, `RetrievalDiagnostics`, and `TokenBudgetEstimate`.
...
2. Include the fields required by `ARCHITECTURE.md`: query, normalized query intent, selected chunk IDs, sanitized excerpts, selected memory item IDs and sanitized summaries when used, citations, source authority ranking, freshness metadata, conflict markers, access-filter result, redaction status, and token budget estimate.
...
4. Represent relationship-derived evidence with relationship path metadata: relationship type, direction, depth, source entity or chunk ID, target entity or chunk ID, and citation IDs for the selected endpoint evidence. Do not include raw edge payloads or unauthorized endpoint metadata.
...
11. Do not include raw source text, raw local cache paths, raw provider payloads, raw embeddings, unauthorized counts, or secrets in the bundle.
...
- Relationship-derived entity and chunk candidates are citation-backed, bounded, and represented without expanding access.
- Diagnostics are useful without leaking unauthorized existence, raw chunks, embeddings, or provider payloads.
