...
6. Register `UnhandledExceptionInstrumentor` once during startup. Cover uncaught process exceptions, uncaught `threading` exceptions, and unhandled asyncio task exceptions.
7. Ensure uncaught exceptions remain visible locally as sanitized structured records and are also exported as OpenTelemetry logs. Avoid duplicate local or remote records when Python's default exception hooks and the OpenTelemetry instrumentor observe the same failure.
...
11. Apply redaction before a record reaches either handler. Do not rely on backend filtering.
...
- Trigger deterministic uncaught process, thread, and asyncio task exceptions in isolated subprocess tests and verify sanitized local and OTLP log records.
...
- `opentelemetry-instrumentation-exceptions` covers uncaught process, thread, and asyncio task exceptions.
