...
- `ARCHITECTURE.md` remains the source of truth for source types, source metadata, corpus eligibility labels, sensitivity labels, and license policy labels.
...
4. Display only catalog metadata: stable source ID, source type, tracked refs or version strategy, extractor profile, source priority, visibility label, corpus eligibility label or corpus eligibility summary, sensitivity class, license policy, refresh cadence, and enabled state.
...
6. Validate that each source carries corpus eligibility, license, and sensitivity fields before it can be listed as enabled.
...
- Every listed source includes visibility, corpus eligibility, sensitivity, and license policy metadata.
